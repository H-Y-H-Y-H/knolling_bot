...
from utils.plots_xyzyaw import my_plot_one_box
from utils.plots_xyzyaw import plot_one_box
...
def eval_img4Batch(img_array, num_obj):
    ...
    log_path = 'model/'
    label = np.loadtxt('Dataset/label/label_217_0180_close100.csv')[:, 2:]
    ...
    model = ResNet50(img_channel=3, output_size=3).to(device)
    # optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    model.load_state_dict(torch.load('Model/best_model_219_ori_poly.pt', map_location='cuda:0'))
    ...
    with torch.no_grad():
        ...
        for batch in test_loader:
            ...
            pred_xyzyaw = pred_xyzyaw.cpu().detach().numpy()
            # print('this is', pred_x1y1x2y2l)
            pred_xyzyaw = scaler.inverse_transform(pred_xyzyaw)
            print('this is', pred_xyzyaw)
...
def Plot4Batch(img, xyxy_list, xy_list, img_label, color_label, obj_num):
    ...
    for i in range(obj_num):
        ...
        info1 = f'this'
        info2 = f'yaw: {my_yaw:.3f}length: {my_length * 1000:.3f} width: {my_width * 1000:.3f}'
        ...
        #                 color=[0, 0, 0], line_thickness=1)
        my_to_arm = [xy[0], xy[1], my_yaw, my_length, my_width, color_label[i]]
...
def detect(cam_img,save_img=False, check_dataset_error=None):
    cam_obs = True
    path = ''
    ...
    for path, img, im0s in dataset:
        ...
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # img = np.clip((1 * img + 70), 0, 255)
        # im0s = np.clip((1 * im0s + 70), 0, 255)
        # cv2.imshow('aaa', img)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # cv2.imshow('bbb', im0s)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # print(box_number)
                im0, to_arm = Plot4Batch(im0, xyxy_list, xy_list, img_label, color_label, box_number)
                # cv2.imshow('123', im0)
                cv2.imwrite('test_219_ori_poly.png',im0)
                # cv2.waitKey(1000)
                if cam_obs:
                    return to_arm
