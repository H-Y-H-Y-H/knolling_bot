...
class Arm:
    ...
    def change_sequence(self, pos, ori, lwh, flag=None):
        if flag == 'distance':
    ...
    def get_parameters(self, box_num=None, area_num=None, ratio_num=None, solution_num=None, kind_num=None,
    ...
    def change_config(self):  # this is main function!!!!!!!!!
        ...
        #                                                                                        self.num_list,
        #                                                                                        self.boxes_index)
        xyz_list = items_sort.get_data_virtual(self.num_list, self.kind_num)
        ...
        lwh_list_epoch = []
        while True:
            self.xyz_list, self.all_index, self.transform_flag = items_sort.judge(xyz_list,self.area_num, self.ratio_num)
            ...
            times += 1
            if times >= self.solution_num:
                break

        # return self.items_pos_list[:, :2], self.items_ori_list[:, 2], self.xyz_list[:, :2], self.transform_flag

        return pos_list_epoch, ori_list_epoch, lwh_list_epoch, None, collect_pos, collect_ori
        ...
        for cfg in range(len(config_dict)):
            ...
            while change_cfg_flag == False:
                ...
                for m in range(solution_num):
                    ...
                    order = env.change_sequence(pos_after, ori_after, xy_after, flag='distance')
