...
class Arm_env(gym.Env):
    ...
    def __init__(self, is_render=True, num_objects=1):
        ...
        self.step_counter = 0
        self.kMaxEpisodeSteps: int = 1000
        ...
        self.action_space = spaces.Box(low=np.array([self.x_low_act, self.y_low_act, self.z_low_act, self.yaw_low_act, self.gripper_low_act]),
                                        ...
                                        dtype=np.float32)
        self.observation_space = spaces.Box(low=np.array([-np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf]),
                                            high=np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]),
                                            dtype=np.float32)
    ...
    def RL_act(self, action):
        ...
        grasp_decision = bool(abs(new_arm_obs[0] - new_box_pos[0]) < (self.x_high_obs - self.x_low_obs)/10 and
        abs(new_arm_obs[1] - new_box_pos[1]) < (self.y_high_obs - self.y_low_obs)/10 and
        (new_arm_obs[2]-new_box_pos[2])< 0.05)
    ...
    def gripper_control(self, cmds):
        ...
        # open gripper
        if cmds == 0:
            logger.info('the distance is ok but the arm does not decide to grasp')
            p.setJointMotorControlArray(self.arm_id, [7, 8], p.POSITION_CONTROL, targetPositions=[0, 0])
        elif cmds == 1:
            while True:
                ...
                if abs(obs_pos[1] - tar_pos[1]) > 0.0005:
                    ...
                    flag = False
                    logger.info(f'sorry, not catch the box, the flag is {flag}')
                    break
        return flag
    ...
    def reward(self):
        ...
        elif abs(box_yaw - ee_yaw) < 0.15:
            r = 0.01
            # logger.info('the yaw is same')
            self.terminated = False
