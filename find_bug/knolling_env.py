...
class Arm_env(gym.Env):
    ...
    def __init__(self,max_step, is_render=True, num_objects=1, x_grasp_accuracy=0.2, y_grasp_accuracy=0.2,
        ...
        self.view_matrix = p.computeViewMatrixFromYawPitchRoll(
            cameraTargetPosition=[0.175, 0, 0.05], #0.175
            distance=0.4,
            yaw=90,
            pitch = -90,
            roll=0,
    ...
    def reset(self):
        ...
        # textureId = p.loadTexture("Dataset/texture/table.jpg")
        ...
        initial_angle = p.calculateInverseKinematics(self.arm_id, 9, targetPosition=[0, 0, 0], maxNumIterations=300,
        ...
        # while True:
        #     rdm_pos_x = np.random.uniform(0.127, 0.233, size =3)
        #     rdm_pos_y = np.random.uniform(-0.08, 0.08, size = 3)
        #
        #
        ...
        for i in range(self.num_objects):
            ...
            # print(lucky)
            self.obj_idx.append(p.loadURDF(("urdf/lego/2x%d.urdf" % lucky), basePosition=[rdm_pos_x[i],rdm_pos_y[i],rdm_pos_z],
                                           baseOrientation=p.getQuaternionFromEuler([0,0,rdm_ori_yaw[i]]),useFixedBase=1,
                                           flags=p.URDF_USE_SELF_COLLISION or p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT))

            # POS=[p.readUserDebugParameter(Debug_para[int(5+2*i)]),p.readUserDebugParameter(Debug_para[int(6+2*i)]),rdm_pos_z]

            # self.obj_idx.append(p.loadURDF(os.path.join(self.urdf_path, "lego/2x%d.urdf" % lucky), basePosition=POS,
            #                                baseOrientation=p.getQuaternionFromEuler([0,0,rdm_ori_yaw[i]]),
            #                                flags=p.URDF_USE_SELF_COLLISION or p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT))
