...
class Arm:
    ...
    def label2image(self, labels_data, img_index, save_urdf_path):
        ...
        recover_offset = [0.016, -0.17 + 0.016, 0]
        ...
        pos_data[:, 0] += recover_offset[0]
        pos_data[:, 1] += recover_offset[1]
    ...
    def change_config(self):  # this is main function!!!!!!!!!
        ...
        # get the standard xyz and corresponding index from files in the computer
        items_sort = Sort_objects()
        xyz_list = items_sort.get_data_virtual(self.num_list)
        ...
        for cfg in range(len(self.area_ratio)):
            ...
            times = 0
            while True:
                self.xyz_list, self.all_index, self.transform_flag = items_sort.judge(xyz_list, self.area_ratio[cfg][0], self.area_ratio[cfg][0])
