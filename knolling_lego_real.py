...
class Arm:
    ...
    def get_parameters(self, num_2x2=0, num_2x3=0, num_2x4=0, num_pencil=0, kinds=4,
                       ...
                       from_virtual=True, real_operate=False):
        ...
        def move(cur_pos, cur_ori, tar_pos, tar_ori, move_slice):
            ...
            if self.real_operate == True:
                while True:
                    ...
                    ik_angles = p.calculateInverseKinematics(self.arm_id, 9, targetPosition=tar_pos,
                                                              maxNumIterations=200,
                    ...
                    for motor_index in range(self.num_motor):
                        p.setJointMotorControl2(self.arm_id, motor_index, p.POSITION_CONTROL,
                                                targetPosition=ik_angles[motor_index], maxVelocity=0.5)
                    for i in range(30):
                        p.stepSimulation()
                        # time.sleep(1 / 480)
                    ...
                    cur_pos = tar_pos
                    cur_ori = tar_ori
                    motor_pos_real = conn.recv(1024)
                    motor_pos_real = np.frombuffer(motor_pos_real, dtype=np.float32)
                    print(f'this is real motor pos {motor_pos_real}')
            else:
                while True:
                    tar_pos = cur_pos + step_pos
                    tar_ori = cur_ori + step_ori
        ...
        def clean_desk():
            ...
            while len(barricade_index) > 0:
                ...
                if self.real_operate == True:
                        ...
                        print(f'this is real reset pos {real_pos}')
                else:
                    pass
