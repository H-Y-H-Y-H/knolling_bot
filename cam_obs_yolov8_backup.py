...
def plot_and_transform(im, box, label='', color=(0, 0, 0), txt_color=(255, 255, 255), index=None, scaled_xylw=None, keypoints=None, use_lw=True, truth_flag=None):
    ...
    if label:
        ...
        if truth_flag == True:
            txt_color = (0, 0, 255)
            im = cv2.putText(im, label1, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2),
                             0, zzz_lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
            im = cv2.putText(im, label2, (p1[0], p1[1] - 12 if outside else p1[1] + h + 12),
                             0, zzz_lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
        else:
            im = cv2.putText(im, label1, (p1[0], p1[1] + 22 if outside else p1[1] + h + 2),
                             0, zzz_lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
            im = cv2.putText(im, label2, (p1[0], p1[1] + 32 if outside else p1[1] + h + 12),
                             0, zzz_lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
...
def yolov8_predict(cfg=DEFAULT_CFG, use_python=False, img_path=None, data_path=None, model_path=None, real_flag=None, target=None):
    ...
    for i in range(len(images)):
        ...
        for elements in one_img:
            ...
            pred_keypoints_label = pred_keypoints_label.reshape(-1, )
            ...
            origin_img, result = plot_and_transform(im=origin_img, box=box.xyxy.squeeze(), label=pred_label, color=(0, 0, 0), txt_color=(255, 255, 255), index=j,
                                            scaled_xylw=pred_xylw, keypoints=pred_keypoints[:, :2], use_lw=use_lw, truth_flag=False)
