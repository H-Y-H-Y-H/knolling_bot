...
class Arm_env(gym.Env):
    ...
    def __init__(self,max_step, is_render=True, num_objects=1, x_grasp_accuracy=0.2, y_grasp_accuracy=0.2,
        ...
        self.view_matrix = p.computeViewMatrixFromYawPitchRoll(
            cameraTargetPosition=[0.15, 0, 0], #0.175
            distance=0.387,
            yaw=90,
            pitch = -90,
            roll=0,
        ...
        if random.uniform(0,1) > 0.5:
            p.configureDebugVisualizer(lightPosition=[random.randint(1,2), random.randint(1,2), 5],
                                       shadowMapResolution=8192, shadowMapIntensity=np.random.randint(3, 7) / 10)
        else:
            p.configureDebugVisualizer(lightPosition=[random.randint(1,2), random.randint(-2,-1), 5],
                                       shadowMapResolution=8192, shadowMapIntensity=np.random.randint(3, 7) / 10)
    ...
    def reset_table(self, close_flag = False):
        ...
        if not close_flag:
            ...
            if random.random() < 0.5:  # x_mode
                ...
                wall_pos = np.random.uniform(0.03, 0.20)
                p.setGravity(-10, 0, -15)
                wallid = p.loadURDF(("plane_2.urdf"), basePosition=[wall_pos, 0, 0],
                                    baseOrientation=p.getQuaternionFromEuler([0, 1.57, 0]), useFixedBase=1,
                                    flags=p.URDF_USE_SELF_COLLISION or p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT)
            else:  # y_mode
                wall_flag = 1

                wall_pos = np.random.uniform(-0.17, 0.12)
        ...
        #     lineToXYZ=[   self.x_low_obs-self.table_boundary, self.y_high_obs+self.table_boundary, self.z_low_obs])
        p.addUserDebugLine(
            lineFromXYZ=[0, -0.17, self.z_low_obs],
            lineToXYZ=[0.3, -0.17, self.z_low_obs])
        p.addUserDebugLine(
            lineFromXYZ=[0, -0.17, self.z_low_obs],
            lineToXYZ=[0, 0.17, self.z_low_obs])
        p.addUserDebugLine(
            lineFromXYZ=[0.3, 0.17, self.z_low_obs],
            lineToXYZ=[0.3, -0.17, self.z_low_obs])
        p.addUserDebugLine(
            lineFromXYZ=[0.3, 0.17, self.z_low_obs],
            lineToXYZ=[0, 0.17, self.z_low_obs])
        ...
        textureId = p.loadTexture(f"img_{background}.png")
        ...
        # for i in range(1):
            ...
            if close_flag:
                ...
                if wall_flag == 0:  # x
                    ...
                    rdm_pos_y = np.random.uniform(-0.15, 0.15, size=(self.num_objects - 1))
    ...
    #     #                     flags=p.URDF_USE_SELF_COLLISION or p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT)
    #     rad_pos = np.random.sample(3)
    #     baseid = p.loadURDF(os.path.join(self.urdf_path, "plane_1.urdf"), basePosition=[0, 0, 0],
    #                         baseOrientation=p.getQuaternionFromEuler([0, 0, 0]), useFixedBase=1,
    #                         flags=p.URDF_USE_SELF_COLLISION or p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT)
